...
```{r}
train$price_range <- ifelse(train$price_range %in% c(0, 1), 0, 1)
```


# 1 - Data Pre processing
```{r}
paste('Number of observations:', dim(train)[1])
paste('Number of columns:', dim(train)[2])
```
...
```{r}
lapply(colnames(train), function(col) histo_plot(train, col, bins=16))
```
...
for (i in 1:total_columns) {
  if (names(train)[i] %in% binary_columns || names(train)[i] == 'price_range') {
    hist(train[[i]], col='green', main=names(train)[i])
  } else {
    hist(train[[i]], main=names(train)[i], col='lightblue')
  }
}
...
```{r}
explanatory_variables <- names(train)[1:19]

# Create scatter plots for each explanatory variable
scatter_plots_list <- lapply(explanatory_variables, function(var) scatter_plot(data=train, 
                                                                               x_var=var,
                                                                               y_var="price_range"))
scatter_plots_list
```
...
log_formula <- formula("price_range ~ ram + battery_power")
```
...
```{r}
priors = normal(location=0, scale=2.5)
```
...
```{r}
pplot <- plot(posteriors, "areas", prob = 0.95, prob_outer = 1)
...
I don't know whether this output is ok or not...
